...
int main() {
    ...
    cout << "2. Enter A: ";
    int A = getValue();
    cout << "   Positive elements <= A: " << countPositiveLessThanA(arr, n, A) << endl;
    
    // Задача 3: последняя пара разных знаков
    int idx = findLastPairWithDifferentSigns(arr, n);
    if (idx == -1) cout << "3. No pair found." << endl;
    else cout << "3. Last pair starts at index: " << idx << endl;
...
void fillArray(int* arr, size_t n) {
    ...
    if (choice == RANDOM) {
        ...
        for (size_t i = 0; i < n; i++) {
            arr[i] = -100 + rand() % 301; // -100..200
        }
    } else if (choice == MANUALY) {
        for (size_t i = 0; i < n; i++) {
            while (true) {
...
int findLastPairWithDifferentSigns(const int* arr, size_t n) {
    for (int i = n - 2; i >= 0; i--) {
        bool sign1 = arr[i] >= 0;
        bool sign2 = arr[i + 1] >= 0;
        if (sign1 != sign2) return i;
    }
    return -1;
}